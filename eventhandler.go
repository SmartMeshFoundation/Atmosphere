...
import (
	...
	"github.com/SmartMeshFoundation/Atmosphere/transfer/mediatedtransfer/initiator"
	"github.com/SmartMeshFoundation/Atmosphere/transfer/mediatedtransfer/target"
...
func (eh *stateMachineEventHandler) dispatchToAllTasks(st transfer.StateChange) {
	for _, mgrs := range eh.atmosphere.Transfer2StateManager {
		eh.dispatch(mgrs, st)
	}
}
...
func (eh *stateMachineEventHandler) dispatchBySecretHash(lockSecretHash common.Hash, st transfer.StateChange) {
	for _, mgr := range eh.atmosphere.Transfer2StateManager {
...
func (eh *stateMachineEventHandler) eventSendRevealSecret(event *mediatedtransfer.EventSendRevealSecret, stateManager *transfer.StateManager) (err error) {
	eh.atmosphere.conditionQuit("EventSendRevealSecretBefore")
	eh.atmosphere.registerSecret(event.Secret)
	revealMessage := encoding.NewRevealSecret(event.Secret)
	err = revealMessage.Sign(eh.atmosphere.PrivateKey, revealMessage)
	err = eh.atmosphere.sendAsync(event.Receiver, revealMessage) //单独处理 reaveal secret
	if err == nil {
		eh.atmosphere.db.UpdateTransferStatus(event.Token, revealMessage.LockSecretHash(), models.TransferStatusCanNotCancel, fmt.Sprintf("RevealSecret 正在发送 target=%s", utils.APex2(event.Receiver)))
	}
	return err
}
func (eh *stateMachineEventHandler) eventSendSecretRequest(event *mediatedtransfer.EventSendSecretRequest, stateManager *transfer.StateManager) (err error) {
	...
	err = secretRequest.Sign(eh.atmosphere.PrivateKey, secretRequest)
	eh.atmosphere.conditionQuit("EventSendSecretRequestBefore")
	ch := eh.atmosphere.getChannelWithAddr(event.ChannelIdentifier)
	if ch == nil {
	...
	if stateManager.LastReceivedMessage == nil {
		...
		err = eh.atmosphere.db.UpdateChannelNoTx(channel.NewChannelSerialization(ch))
	} else {
		eh.atmosphere.updateChannelAndSaveAck(ch, stateManager.LastReceivedMessage.Tag())
		stateManager.LastReceivedMessage = nil
	}
	err = eh.atmosphere.sendAsync(event.Receiver, secretRequest)
	return
}
func (eh *stateMachineEventHandler) eventSendMediatedTransfer(event *mediatedtransfer.EventSendMediatedTransfer, stateManager *transfer.StateManager) (err error) {
	...
	g := eh.atmosphere.getToken2ChannelGraph(event.Token)
	...
	err = mtr.Sign(eh.atmosphere.PrivateKey, mtr)
	err = ch.RegisterTransfer(eh.atmosphere.GetBlockNumber(), mtr)
	if err != nil {
		return
	}
	eh.atmosphere.conditionQuit("EventSendMediatedTransferBefore")
	if stateManager.LastReceivedMessage == nil {
		...
		fromCh, err = eh.atmosphere.findChannelByIdentifier(event.FromChannel)
		if err != nil {
			return
		}
		t, _ := stateManager.LastReceivedMessage.Tag().(*transfer.MessageTag)
		echohash := t.EchoHash
		ack := eh.atmosphere.Protocol.CreateAck(echohash)
		tx := eh.atmosphere.db.StartTx()
		eh.atmosphere.db.SaveAck(echohash, ack.Pack(), tx)
		err = eh.atmosphere.db.UpdateChannel(channel.NewChannelSerialization(ch), tx)
		err = eh.atmosphere.db.UpdateChannel(channel.NewChannelSerialization(fromCh), tx)
		if err != nil {
	...
	err = eh.atmosphere.sendAsync(receiver, mtr)
	if err == nil {
		eh.atmosphere.db.UpdateTransferStatus(ch.TokenAddress, mtr.LockSecretHash, models.TransferStatusCanCancel, fmt.Sprintf("MediatedTransfer 正在发送 target=%s", utils.APex2(receiver)))
	}
	return
}
func (eh *stateMachineEventHandler) eventSendUnlock(event *mediatedtransfer.EventSendBalanceProof, stateManager *transfer.StateManager) (err error) {
	...
	err = tr.Sign(eh.atmosphere.PrivateKey, tr)
	err = ch.RegisterTransfer(eh.atmosphere.GetBlockNumber(), tr)
	if err != nil {
		return
	}
	eh.atmosphere.conditionQuit("EventSendUnlockBefore")
	err = eh.atmosphere.db.UpdateChannelNoTx(channel.NewChannelSerialization(ch))
	err = eh.atmosphere.sendAsync(receiver, tr)
	if err == nil {
		eh.atmosphere.db.UpdateTransferStatusMessage(event.Token, event.LockSecretHash, fmt.Sprintf("Unlock 正在发送 target=%s", utils.APex2(receiver)))
	}
	return
}
func (eh *stateMachineEventHandler) eventSendAnnouncedDisposed(event *mediatedtransfer.EventSendAnnounceDisposed, stateManager *transfer.StateManager) (err error) {
	...
	g := eh.atmosphere.getToken2ChannelGraph(event.Token)
	ch := g.GetPartenerAddress2Channel(receiver)
	mtr, err := ch.CreateAnnouceDisposed(event.LockSecretHash, eh.atmosphere.GetBlockNumber())
	if err != nil {
		return
	}
	err = mtr.Sign(eh.atmosphere.PrivateKey, mtr)
	...
	err = eh.atmosphere.db.MarkLockSecretHashDisposed(event.LockSecretHash, ch.ChannelIdentifier.ChannelIdentifier)
	if err != nil {
		return
	}
	if stateManager.LastReceivedMessage == nil {
	...
	eh.atmosphere.conditionQuit("EventSendAnnouncedDisposedBefore")
	err = eh.atmosphere.sendAsync(receiver, mtr)
	return
}
func (eh *stateMachineEventHandler) eventSendAnnouncedDisposedResponse(event *mediatedtransfer.EventSendAnnounceDisposedResponse, stateManager *transfer.StateManager) (err error) {
	...
	g := eh.atmosphere.getToken2ChannelGraph(event.Token)
	ch := g.GetPartenerAddress2Channel(receiver)
	mtr, err := ch.CreateAnnounceDisposedResponse(event.LockSecretHash, eh.atmosphere.GetBlockNumber())
	if err != nil {
		return
	}
	err = mtr.Sign(eh.atmosphere.PrivateKey, mtr)
	err = ch.RegisterAnnounceDisposedResponse(mtr, eh.atmosphere.GetBlockNumber())
	if err != nil {
		return
	}
	eh.atmosphere.conditionQuit("EventSendAnnouncedDisposedResponseBefore")
	if stateManager.LastReceivedMessage == nil {
...
func (eh *stateMachineEventHandler) eventContractSendRegisterSecret(event *mediatedtransfer.EventContractSendRegisterSecret) (err error) {
	b, err := eh.atmosphere.Chain.SecretRegistryProxy.IsSecretRegistered(event.Secret)
	if err != nil {
		return err
	}
	if b {
	...
	result := eh.atmosphere.Chain.SecretRegistryProxy.RegisterSecretAsync(event.Secret)
	go func() {
		var err error
		err = <-result.Result
		if err != nil {
...
func (eh *stateMachineEventHandler) eventUnlockFailed(e2 *mediatedtransfer.EventUnlockFailed, manager *transfer.StateManager) (err error) {
	...
	tr, err := ch.CreateRemoveExpiredHashLockTransfer(e2.LockSecretHash, eh.atmosphere.GetBlockNumber())
	if err != nil {
		log.Warn(fmt.Sprintf("Get Event UnlockFailed ,but hashlock cannot be removed err:%s", err))
		return
	}
	err = tr.Sign(eh.atmosphere.PrivateKey, tr)
	err = ch.RegisterRemoveExpiredHashlockTransfer(tr, eh.atmosphere.GetBlockNumber())
	if err != nil {
		log.Error(fmt.Sprintf("register mine RegisterRemoveExpiredHashlockTransfer err %s", err))
		return
	}
	eh.atmosphere.conditionQuit("EventRemoveExpiredHashlockTransferBefore")
	err = eh.atmosphere.db.UpdateChannelNoTx(channel.NewChannelSerialization(ch))
	err = eh.atmosphere.sendAsync(ch.PartnerState.Address, tr)
	eh.atmosphere.db.UpdateTransferStatus(ch.TokenAddress, e2.LockSecretHash, models.TransferStatusFailed, fmt.Sprintf("交易超时失败 err=%s", e2.Reason))
	return
}
...
func (eh *stateMachineEventHandler) eventSaveFeeChargeRecord(e *mediatedtransfer.EventSaveFeeChargeRecord) (err error) {
	...
	return eh.atmosphere.db.SaveFeeChargeRecord(r)
}
...
func (eh *stateMachineEventHandler) OnEvent(event transfer.Event, stateManager *transfer.StateManager) (err error) {
	...
	case *mediatedtransfer.EventSendMediatedTransfer:
		...
		eh.atmosphere.conditionQuit("EventSendMediatedTransferAfter")
	case *mediatedtransfer.EventSendRevealSecret:
		err = eh.eventSendRevealSecret(e2, stateManager)
		eh.atmosphere.conditionQuit("EventSendRevealSecretAfter")
	...
	case *transfer.EventTransferSentFailed:
		eh.atmosphere.db.UpdateTransferStatus(e2.Token, e2.LockSecretHash, models.TransferStatusFailed, fmt.Sprintf("交易失败 err=%s", e2.Reason))
		eh.finishOneTransfer(event)
	...
	case *mediatedtransfer.EventUnlockFailed:
		...
		eh.atmosphere.conditionQuit("EventSendRemoveExpiredHashlockTransferAfter")
	...
	case *mediatedtransfer.EventRemoveStateManager:
		delete(eh.atmosphere.Transfer2StateManager, e2.Key)
...
func (eh *stateMachineEventHandler) finishOneTransfer(ev transfer.Event) {
	...
	if lockSecretHash != utils.EmptyHash {
		...
		r := eh.atmosphere.Transfer2Result[smkey]
		...
		r.Result <- err
		delete(eh.atmosphere.Transfer2Result, smkey)
	}
}
func (eh *stateMachineEventHandler) handleTokenAdded(tokenAddress common.Address) error {
	//	????
	log.Info(fmt.Sprintf("NewTokenAdd token=%s", tokenAddress.String()))
	err := eh.atmosphere.db.AddToken(tokenAddress)
	if err != nil {
		return err
	}
	g := graph.NewChannelGraph(eh.atmosphere.NodeAddress, tokenAddress, nil)
	eh.atmosphere.TokenAddressMap[tokenAddress] = true
	eh.atmosphere.Token2ChannelGraph[tokenAddress] = g
	return nil
}
func (eh *stateMachineEventHandler) handleChannelNew(st *mediatedtransfer.ContractNewChannelStateChange) error {
	...
	log.Info(fmt.Sprintf("NewChannel channel=%s, token=%s,participant1=%s,participant2=%s",
		utils.HPex(st.ChannelIdentifier.ChannelIdentifier),
	...
	/*
		如果该token是第一次出现
	*/
	if !eh.atmosphere.TokenAddressMap[tokenAddress] {
		err := eh.handleTokenAdded(tokenAddress)
		if err != nil {
			log.Error(err.Error())
			return err
		}
	}
	g := eh.atmosphere.getToken2ChannelGraph(tokenAddress)
	g.AddPath(participant1, participant2)
	err := eh.atmosphere.db.NewNonParticipantChannel(tokenAddress, st.ChannelIdentifier.ChannelIdentifier, participant1, participant2)
	if err != nil {
		log.Error(err.Error())
		return err
	}
	isParticipant := eh.atmosphere.NodeAddress == participant2 || eh.atmosphere.NodeAddress == participant1
	partner := st.Participant1
	if partner == eh.atmosphere.NodeAddress {
		partner = st.Participant2
	}
	if isParticipant {
		eh.atmosphere.registerChannel(tokenAddress, partner, st.ChannelIdentifier, st.SettleTimeout)
		other := participant2
		if other == eh.atmosphere.NodeAddress {
			other = participant1
		}
		eh.atmosphere.startHealthCheckFor(other)
	} else {
...
func (eh *stateMachineEventHandler) handleBalance(st *mediatedtransfer.ContractBalanceStateChange) error {
	ch, err := eh.atmosphere.findChannelByIdentifier(st.ChannelIdentifier)
	if err != nil {
	...
	err = eh.atmosphere.db.UpdateChannelContractBalance(channel.NewChannelSerialization(ch))
	return nil
}
...
func (eh *stateMachineEventHandler) handleClosed(st *mediatedtransfer.ContractClosedStateChange) error {
	...
	ch, err := eh.atmosphere.findChannelByIdentifier(channelIdentifier)
	if err != nil {
		//i'm not a participant
		// 如果不是自己参与的channel,移除路由中的path
		tokenAddress, _, p1, p2, err2 := eh.atmosphere.db.GetNonParticipantChannelByID(channelIdentifier)
		if err2 != nil {
			return err2
		}
		g := eh.atmosphere.getToken2ChannelGraph(tokenAddress)
		...
		return eh.atmosphere.db.RemoveNonParticipantChannel(st.ChannelIdentifier)
	}
	err = eh.ChannelStateTransition(ch, st)
	if err != nil {
		log.Error(fmt.Sprintf("handleBalance ChannelStateTransition err=%s", err))
	}
	err = eh.atmosphere.db.UpdateChannelState(channel.NewChannelSerialization(ch))
	return err
}
...
func (eh *stateMachineEventHandler) removeSettledChannel(ch *channel.Channel) error {
	g := eh.atmosphere.getChannelGraph(ch.ChannelIdentifier.ChannelIdentifier)
	g.RemoveChannel(ch)
	cs := channel.NewChannelSerialization(ch)
	err := eh.atmosphere.db.RemoveChannel(cs)
	if err != nil {
		return err
	}
	err = eh.atmosphere.db.NewSettledChannel(cs)
	if err != nil {
		return err
	}
	err = eh.atmosphere.db.RemoveNonParticipantChannel(ch.ChannelIdentifier.ChannelIdentifier)
	return err
}
...
func (eh *stateMachineEventHandler) handleCooperativeSettled(st *mediatedtransfer.ContractCooperativeSettledStateChange) error {
	...
	if err != nil {
		//i'm not a participant
		// 如果不是自己参与的channel,移除路由中的path
		tokenAddress, _, p1, p2, err2 := eh.atmosphere.db.GetNonParticipantChannelByID(st.ChannelIdentifier)
		if err2 != nil {
			return err2
		}
		g := eh.atmosphere.getToken2ChannelGraph(tokenAddress)
...
func (eh *stateMachineEventHandler) handleWithdraw(st *mediatedtransfer.ContractChannelWithdrawStateChange) error {
	...
	ch, err := eh.atmosphere.findChannelByIdentifier(st.ChannelIdentifier.ChannelIdentifier)
	if err != nil {
		return nil
	}
	err = eh.ChannelStateTransition(ch, st)
...
func (eh *stateMachineEventHandler) handleUnlockOnChain(st *mediatedtransfer.ContractUnlockStateChange) error {
	...
	if eh.atmosphere.NodeAddress == st.Participant {
		ad := eh.atmosphere.db.GetReceiviedAnnounceDisposed(st.LockHash, ch.ChannelIdentifier.ChannelIdentifier)
		if ad != nil {
			result := ch.ExternState.PunishObsoleteUnlock(ch.TokenAddress, common.BytesToHash(ad.LockHash), ad.AdditionalHash, ad.Signature)
			go func() {
				var err2 error
				err2 = <-result.Result
				if err2 != nil {
...
func (eh *stateMachineEventHandler) handleSecretRegisteredOnChain(st *mediatedtransfer.ContractSecretRevealOnChainStateChange) error {
	...
	eh.atmosphere.registerRevealedLockSecretHash(st.LockSecretHash, st.Secret, st.BlockNumber)
...
func (eh *stateMachineEventHandler) updateStateManagerFromStateChange(mgr *transfer.StateManager, stateChange transfer.StateChange) {
	...
	if len(quitName) > 0 {
		eh.atmosphere.conditionQuit(quitName)
	}
}
