...
func GenerateParams(BitCurve *secp256k1.BitCurve, primeCertainty int32, kPrime int32, paillierPubKey *PublicKey) *PublicParameters {
	var p, q, pPrime, qPrime, pPrimeqPrime, nHat *big.Int
	for {
...
func RandomFromZn2(p *big.Int) *big.Int {

	r, err := crand.Int(crand.Reader, p)
	if err != nil {
...
func PointMul(scalar *big.Int, p *Point) *Point {
	r := &Point{}
	log.Info(fmt.Sprintf("PointMul bitlen=%d", scalar.BitLen()))
