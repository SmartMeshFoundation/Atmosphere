...
func (ecds *ECDSASignature) NewECDSASignature(r *big.Int, s *big.Int) {
	ecds.r = r
	ecds.s = s
}

func (ecds *ECDSASignature) NewECDSASignatureWithV(r *big.Int, s *big.Int, recoveryParam int32) {
	ecds.r = r
	ecds.s = s
...
func (ecds *ECDSASignature) Verify(message []byte, pkx *big.Int, pky *big.Int) bool {
	...
	if xR.Cmp(ecds.r) == 0 {
		log.Info("ECDSA Signature Verify Passed! (r,s) is a Valid Siganture!")
		log.Info(fmt.Sprintf("(r,s) r=%s", ecds.r))
		log.Info(fmt.Sprintf("(r,s) s=%s", ecds.s))
		return true
	}

	log.Error("ERROR: ECDSA Signature Verify NOT Passed! ")
	return false
}
