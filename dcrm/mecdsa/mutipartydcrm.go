...
func KeyGenTN(t, n int) (party_keys_vec []*Keys, shared_keys_vec []*SharedKeys, pk_vec []*share.SPubKey, total_pubkey *share.SPubKey, vss0 *feldman.VerifiableSS, err error) {
	parames := &Parameters{
		Threshold:  t,
		ShareCount: n,
	}
	...
	var y_vec []*share.SPubKey //私钥片对应的公钥集合
	...
	for i := 1; i < len(y_vec); i++ {
		total_pubkey.X, total_pubkey.Y = share.PointAdd(total_pubkey.X, total_pubkey.Y, y_vec[i].X, y_vec[i].Y)
	}
	//shamir 配多项式
	var vss_scheme_vec []*feldman.VerifiableSS //vss
	var secret_shares_vec [][]share.SPrivKey   //feldman vss提供
	var index_vec []int
	for i := 0; i < n; i++ {
		/*
	...
	var partyShares [][]share.SPrivKey //重新组合的secret shares
	for i := 0; i < n; i++ {
		var s []share.SPrivKey
		for j := 0; j < n; j++ {
	...
	for i := 0; i < len(party_keys_vec); i++ {
		share.ModAdd(sum, party_keys_vec[i].ui)
	}
	if x.D.Cmp(sum.D) != 0 {
		log.Trace(fmt.Sprintf("x=%s", x))
		log.Trace(fmt.Sprintf("sum=%s", sum))
	...
	tx, ty := share.S.ScalarBaseMult(x.Bytes())
	if tx.Cmp(total_pubkey.X) != 0 || ty.Cmp(total_pubkey.Y) != 0 {
		return nil, nil, nil, nil, nil, errors.New("pubkey error")
	}
...
//func genKey() (party_keys_vec []*Keys, shared_keys_vec []*SharedKeys, pk_vec []*share.SPubKey, total_pubkey *share.SPubKey, vss0 *share.VerifiableSS) {
//	/// 生成五个key
//	//1
//	k := &Keys{}
//	k.ui = share.BigInt2PrivateKey(strHex2bigint("2f7a3834a7201c47ade1ae354fc0985cee98993dd97b8878d3214745c8fb1a6c"))
//	yix, yiy := share.S.ScalarBaseMult(k.ui.Bytes())
//	k.yi = share.NewGE(yix, yiy)
...
//	//五个sharedKeys
//	var sk *SharedKeys
//	yix, yiy = share.Strtoxy("23946e1ef0bcf8a1b926d445be4f0054bb7f20677d1669a9f31d45e4027a9ae8f5fb9a8c91a2f580e93fa053d2d25310269ded4be2e0516e217382d6822c1ab0")
//	sk = &SharedKeys{}
//	sk.xi = strHex2bigint("bb6bf2c8705a844228c6cf3866f885901741fe1bcb7197905650be6fec731fee")
//	sk.y = share.NewGE(yix, yiy)
...
//
//	yix, yiy = share.Strtoxy("23946e1ef0bcf8a1b926d445be4f0054bb7f20677d1669a9f31d45e4027a9ae8f5fb9a8c91a2f580e93fa053d2d25310269ded4be2e0516e217382d6822c1ab0")
//	total_pubkey = share.NewGE(yix, yiy)
//
//	vss0 = &share.VerifiableSS{
//		Parameters:  share.ShamirSecretSharing{2, 5},
//		Commitments: make([]*share.SPubKey, 3),
//	}
//	yix, yiy = share.Strtoxy("4c25ebabf0d4f79c6373520d66c05fa00ed5d316279419d6f3550e356b1ca17dd3c591c492f9c42cbc9532e4e39844f1fbe472b842666390c45ee0d5456aa2d3")
//	vss0.Commitments[0] = share.NewGE(yix, yiy)
//	yix, yiy = share.Strtoxy("9817f8165b81f259d928ce2ddbfc9b02070b87ce9562a055acbbdcf97e66be79b8d410fb8fd0479c195485a648b417fda808110efcfba45d65c4a32677da3a48")
//	vss0.Commitments[1] = share.NewGE(yix, yiy)
//	yix, yiy = share.Strtoxy("e59e705cb909acaba73cef8c4b8e775cd87cc0956e4045306d7ded41947f04c62ae5cf50a9316423e1d066326532f6f7eeea6c461984c5a339c33da6fe68e11a")
//	vss0.Commitments[2] = share.NewGE(yix, yiy)
//	yix.String()
//	return
//}
...
//	log.Trace(fmt.Sprintf("y=%s", share.Xytostr(y.X, y.Y)))
...
//	}
//	var tx, ty *big.Int
//	sign_keys_vec[0].ki =share.BigInt2PrivateKey(  strHex2bigint("b33478cfe5717369cda7ad324f433f94e94dd351236996d54974b9c476839e8a"))
//	sign_keys_vec[0].gammaI =share.BigInt2PrivateKey( strHex2bigint("1aa79e0dd371048b867ca12d5ecf9d120795146570c20a69a877db8f64e5bb51"))
//	tx, ty = share.Strtoxy("ea177ffb3c92346ef229a50d8ed53459f4857f3ccddddc5ff12ad8da55d29300313a58032723460dce5e8881fd97bceff3d04a429814fad8dd62974f0ae0be06")
//	sign_keys_vec[0].gGammaI = share.NewGE(tx, ty)
//
//	sign_keys_vec[1].ki =share.BigInt2PrivateKey( strHex2bigint("f7be401f0800ca965b446d5bd6966bf7bc94bec8d5f43d2e2b4f5dcbe5665b6b"))
//	sign_keys_vec[1].gammaI = share.BigInt2PrivateKey(strHex2bigint("77fb64a960cfa225c3035bcfe2907bd5fedc0b632b9b346cfa859580cc83eede"))
//	tx, ty = share.Strtoxy("da6bea1a140421992daa74230b18896a7a03128d1efce22e2e7bf168daa0ba27290c1e3c031e6c17139f2a465f42b483fdf5f8f79a0950a097be0abef15eb830")
//	sign_keys_vec[1].gGammaI = share.NewGE(tx, ty)
//
//	sign_keys_vec[2].ki = share.BigInt2PrivateKey(strHex2bigint("9e5118fc0fc1fecad68a727d723eab8608a8420d95b7af92456396423adaadb5"))
//	sign_keys_vec[2].gammaI = share.BigInt2PrivateKey(strHex2bigint("05985e6c2aa6859b07cab846e339e2edf6cb44d1baaf41352e04738b0df4db1d"))
//	tx, ty = share.Strtoxy("c2b6f70f6de4148cf2be87772178d278c4d9c1ed5e5f998fc2990aa769cf06d03b2e7232a5b2b57556e9553dcfd07b0055f1a9d146906036357f780ad273da78")
//	sign_keys_vec[2].gGammaI = share.NewGE(tx, ty)
//
//	sign_keys_vec[3].ki = share.BigInt2PrivateKey(strHex2bigint("1d6bb049eb7cc4885eec57bc36f2cc8210ba7cf97ee65a735d977bb0591197b1"))
//	sign_keys_vec[3].gammaI = share.BigInt2PrivateKey(strHex2bigint("bed1667cb4ce4a3379248ca21dc8f21db6004dc19804dbde03b6c90b419bd7f3"))
//	tx, ty = share.Strtoxy("ed94a04e2594257b2159c30dd4a7ed168ba3d63221f7a877243e3ef0c047d5136a14b4f74583a4ec62c0f39d628f31a5d506d0503a492c1ffad17187f86a6e26")
//	sign_keys_vec[3].gGammaI = share.NewGE(tx, ty)
//
...
func Sign2(t, n, ttag int, s []int) error {
	...
	// aggregation of the n messages of all parties
	var m_b_gamma_vec_all [][]*MessageB
	var beta_vec_all [][]share.SPrivKey
	var m_b_w_vec_all [][]*MessageB
	var ni_vec_all [][]share.SPrivKey

	for i := 0; i < ttag; i++ {
		var m_b_gamma_vec []*MessageB
		var beta_vec []share.SPrivKey
		var m_b_w_vec []*MessageB
		var ni_vec []share.SPrivKey
		for j := 0; j < ttag-1; j++ {
			ind := j
			if j >= i {
				ind = j + 1
	...
	// IRL each party i should get only the mb messages that other parties sent in response to the party i ma's.
	// TODO: simulate as IRL
	var alpha_vec_all [][]share.SPrivKey
	var miu_vec_all [][]share.SPrivKey
	for i := 0; i < ttag; i++ {
		var alpha_vec []share.SPrivKey
		var miu_vec []share.SPrivKey
	...
	var delta_vec []share.SPrivKey
	var sigma_vec []share.SPrivKey
